...
def ucs(start, goal):
    ...
    while frontier:
        ...
        if current == goal:
            end_time = time.time()
            return reconstruct_path(came_from, tuple(map(tuple, current))), nodes_expanded, cost_so_far[tuple(map(tuple, current))], (end_time - start_time) * 1000
...
#
#
# path, nodes_expanded, depth, runtime = ucs(initial_state, goal_state)
#
# if path:
